<?php
		...
		use ROBOAMP\Utils\Filename_helper;
		
		
		class Directory extends Filename_helper {
				 ...
				 * @param string|null $filenames_starting_with_string Optional prefix to filter file names by
				 * @return array An array of information about each file in the directory
				 */
				public function get_files_info_in_directory($directory, $filenames_starting_with_string = null) {
						// Use glob to find files in the directory
						if (is_null($filenames_starting_with_string)) {
								// If no prefix is specified, get all files in the directory
								$files = glob($directory . '/*');
						} else {
								// If a prefix is specified, filter files by prefix
								$files = glob($directory . '/' . $filenames_starting_with_string . '*');
						}
						...
						foreach ($files as $file) {
								if (is_file($file)) {
										$info = pathinfo($file);
										$info['name'] = explode(".", $info['filename'])[0];
										$files_array[] = $info;
								}
						}
						return $files_array;
				 ...
				 * @param bool $show_extension Whether to include file extensions in the names.
				 * @return array The array of file names.
				 */
				public function get_files_names_in_directory($directory, $show_extension = false) {
						$res = $this->get_files_info_in_directory($directory);
						$files = [];
						...
						foreach ($res as $item) {
								if ($show_extension) {
										...
										// Exclude the file extension from the name
										$files[] = $item['name'];
								}
						}
				...
				public static function get_files_in_directory($directory) {
						if (!is_dir($directory)) {
								dd('Error from ' . __METHOD__, 'Directory ' . $directory . ' doesn\'t exist');
						}
						$manuals = [];
						...
						foreach ($filesInFolder as $path) {
								...
								$manuals[$j]['created_at'] = date("F d Y", @filemtime($manuals[$j]['full_path']));
				...
				public function get_dirs_in_dir($directory) {
						...
						foreach (scandir($directory) as $file) {
								...
								$dir = $directory . DIRECTORY_SEPARATOR . $file;
								if (is_dir($dir)) {
										$directories [] = $dir;
								}
						}
				...
				public function get_files_in_directories(string $directory, array $exclude_directories = [], $file = null) {
						
						$dirs_in_dirs = $this->get_dirs_in_dir($directory, $file);
						...
						foreach ($directory as $item) {
								...
								if ($valid_directory) {
										...
										$arr[$j] = $this::get_files_in_directory($path);
